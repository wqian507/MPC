...
def run_mpc_cifar(
    ...
    if evaluate:
        if not skip_plaintext:
            logging.info(f"===== Evaluating plaintext {net} network =====")
            validate(val_loader, model, criterion, print_freq)
        logging.info(f"===== Evaluating Private {net} network =====")
        input_size = get_input_size(val_loader, batch_size)
        private_model = construct_private_model(net, input_size, model)
        validate(device, val_loader, private_model, criterion, print_freq)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # train for one epoch
        train(device,train_loader, model, criterion, optimizer, epoch, print_freq)

        # evaluate on validation set
        prec1 = validate(device, val_loader, model, criterion, print_freq, epoch)
...
def train(device, train_loader, model, criterion, optimizer, epoch, print_freq=10):
...
def construct_private_model(net, input_size, model):
...
def validate(device, val_loader, model, criterion, print_freq=10, epoch=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
...
def save_checkpoint(state, is_best, net, device, filename="_checkpoint.pth.tar", modelname="_model_best.pth.tar"):
    """Saves checkpoint of plaintext model"""
    # only save from rank 0 process to avoid race condition
    filename = device + "_" + net + filename
    modelname = device + "_" + net + modelname
    rank = comm.get().get_rank()
    if rank == 0:
